...
src_dir="/home/earajr/WOEST/data/${strt_date}${strt_hour}"
base_dest_dir="/home/earajr/WOEST/images/${project}/${region}/${strt_date}/${strt_hour}"
      ...
      while IFS= read -r var_line; do
         ...
         then
	       ...
	       dest_dir="${base_dest_dir}/${dom}/maps/${var1}/"
               ...
               echo "python ${script_dir}/../WRF_python/map_${var1}.py ${fil} ${dest_dir} ${map_name} ${limit_lat1} ${limit_lat2} ${limit_lon1} ${limit_lon2}" >> ${command_list}
            done
         ...
         then
	       ...
	       for plev in "${plevs[@]}"; do
	          ...
	          if [ ! -d ${dest_dir} ]
                  ...
                  echo "python ${script_dir}/../WRF_python/map_${var}.py ${fil} ${dest_dir} p${plev} ${map_name} ${limit_lat1} ${limit_lat2} ${limit_lon1} ${limit_lon2}" >> ${command_list}
               done
   	    done
         ...
         then
            ...
            for var_alevs in "${vars_alevs[@]}"; do
               ...
               for alev in "${alevs[@]}"; do
                  ...
                  echo "python ${script_dir}/../WRF_python/map_${var}.py ${fil} ${dest_dir} a${alev} ${map_name} ${limit_lat1} ${limit_lat2} ${limit_lon1} ${limit_lon2}" >> ${command_list}
               done
            done
         fi
      done < ${namelist_vars}
            ...
            for domain in "${domains[@]}"; do
               ...
               then
                  while IFS= read -r profile_info_line; do
                     ...
                     echo "python ${script_dir}/../WRF_python/profile_${var}.py ${fil} ${dest_dir} ${profile_lat} ${profile_lon} ${profile_name}" >> ${command_list}
   
                     if [ ! -f "${base_dest_dir}/${domain}/profiles/${profile_name}/profilelocation_${dom}_${profile_name}.png" ]
                     then
		        echo "python ${script_dir}/../WRF_python/map_profilelocation.py ${fil} ${base_dest_dir}/${domain}/profiles/${profile_name} ${profile_name} ${profile_lat} ${profile_lon}" >> ${command_list}
                     fi
                  done < ${profile_info}
               fi
            done
...
# CROSSSECTION PLOTS
   ...
   while IFS= read -r crosssection_info_line; do
      ...
      then
         echo "python ${script_dir}/../WRF_python/map_crosssectionlocation.py ${fil} ${dest_dir} ${crosssection_name} $( echo ${lats} | tr : , ) $( echo ${lons} | tr : , )" >> ${command_list}
      fi
      ...
      while IFS= read -r var_line; do
         ...
         then
            ...
            for var_domains in "${vars_domains[@]}"; do
		     ...
		     echo "python ${script_dir}/../WRF_python/crosssection_${var}.py ${fil} ${dest_dir} $( echo ${lats} | tr : , ) $( echo ${lons} | tr : , ) 0,0 ${base_alt} ${top_alt} ${crosssection_name}" >> ${command_list}
                  fi
               done
            done
         fi
               ...
               for var_domains in "${vars_domains[@]}"; do
                  ...
                  for domain in "${domains[@]}"; do
		           ...
		           if [ ! -d ${dest_dir} ]
                           ...
                           echo "python ${script_dir}/../WRF_python/timeline_${var}.py ${fil} ${dest_dir} ${timeline_lat} ${timeline_lon} ${timeline_name} ${timeline_starttime} ${timeline_length}" >> ${command_list}
		        done < ${timeline_info}
                     fi
		  done
	       done
