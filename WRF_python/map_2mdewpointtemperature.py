...
# Create inset colourbar
         ...
         if cbar_inset:
            ...
            if portrait:
               ...
               cbbox.tick_params(axis='both', left=False, top=False, right=False, bottom=False, labelleft=False, labeltop=False, labelright=False, labelbottom=False)
               cbbox.set_facecolor([1,1,1,0.7])
               cbbox.text(0.5,0.3, "2m dewpoint temperature ($^\circ$C)", verticalalignment='center', horizontalalignment='center')
...
# Loop through maps, create input dictionary for each map and pass it to the map_2mdewpointtemperature function above
   ...
   for i in np.arange(0, np.shape(limit_lats)[0], 1):
      ...
      plt.savefig(dest_dir+"/2mdewpointtemperaturetest_"+map_names[i][0]+".png", bbox_inches='tight')
