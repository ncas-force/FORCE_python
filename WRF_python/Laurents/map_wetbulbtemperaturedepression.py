def map_wetbulbtemperaturedepression(x):
   import numpy as np
...
# Read projection from a variable (will be able to detect all possible WRF projections and use them for plotting) also identify the required max and min indices
      if x1_y1[1] >= 0 and x1_y1[1] < np.shape(slp_all)[0] and x1_y1[0] >= 0 and x1_y1[0] < np.shape(slp_all)[1] and x2_y2[1] >= 0 and x2_y2[1] < np.shape(slp_all)[0] and x2_y2[0] >= 0 and x2_y2[0] < np.shape(slp_all)[1]:
         ...
         twb = getvar(wrf_in, 'twb', timeidx=i, units="degC")[:,min_lat_ind:max_lat_ind+1, min_lon_ind:max_lon_ind+1]
...
# Interpolate to level on correct level type
         ...
         if leveltype == "pressure":
            twbd_level = interplevel(twbd, pres, levels)
         if leveltype == "altitude":
            twbd_level = interplevel(twbd, z, levels)
...
# Plot wetbulb temperature
         ...
         twbd_lvl2 = np.arange(0.0, 10.0, 1.0)
         twbd_contour = plt.contour(lons, lats, twbd_level,levels=twbd_lvl2, colors='k', transform=crs.PlateCarree())
         plt.clabel(twbd_contour, inline=1, fontsize=10, fmt="%.0f")

         twbd_lvl = np.arange(0.0, 10.5, 0.5)
...
#         mymap = mcolors.LinearSegmentedColormap.from_list('my_colormap', colors)
         plt.contourf(lons, lats, twbd_level, levels=twbd_lvl, zorder=1, cmap='GnBu_r', transform=crs.PlateCarree())
...
# Create inset colourbar
         ...
         if leveltype == "pressure":
            colorbartext= "%.0f" %levels +" hPa wet bulb temperature depression ($^\circ$C)"
         if leveltype == "altitude":
            colorbartext= "%.0f" %levels +" m wet bulb temperature depression ($^\circ$C)"
...
#  dest_dir = "/home/earajr/FORCE_WRF_plotting/output/wetbulbtemperature"
   dest_dir = sys.argv[2]
...
# Create input dictionary for each map and pass it to the map_wetbulbtemperaturedepression function above
   ...
   fig = map_wetbulbtemperaturedepression(input_dict)

   plt.savefig(dest_dir+"/wetbulbtemperaturedepression_"+dom+"_"+date+"_"+time+"_"+level_flag+"_"+map_names[0]+".png", bbox_inches='tight')
