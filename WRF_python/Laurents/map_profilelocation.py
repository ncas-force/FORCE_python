...
# Check that the supplied lat and lon values are within the WRF domain, if not then the plot will not be created.
   ...
   x_index = to_np(ll_to_xy(wrf_in, lat, lon))[0]
   y_index = to_np(ll_to_xy(wrf_in, lat, lon))[1]
...
# Read projection from a variable (will be able to detect all possible WRF projections and use them for plotting) also identify the required max and min indices 
   ...
   points_in_dom = True
   if y_index >= 0 and y_index < np.shape(ter)[0] and x_index >= 0 and x_index < np.shape(ter)[1]:
      print("Point passed")
   else:
      print("Point did not pass")
      points_in_dom = False
...
# Add inset title
      ...
      if title_inset:
         ...
         titlebox.text(0.5,0.5, profile_name+" position", rotation=90.0, verticalalignment='center', horizontalalignment='center')
      else:
         titlebox = inset_axes(ax, '100%', '100%', bbox_to_anchor=(0, -0.05, 1, 0.05), bbox_transform=ax.transAxes, loc = 8, borderpad=0)
         [titlebox.spines[k].set_visible(False) for k in titlebox.spines]
         titlebox.tick_params(axis='both', left=False, top=False, right=False, bottom=False, labelleft=False, labeltop=False, labelright=False, labelbottom=False)
         titlebox.set_facecolor([1,1,1,0.7])
         titlebox.text(0.5,0.5, profile_name+" position", rotation=0.0, verticalalignment='center', horizontalalignment='center')
      ...
      ax.plot(float(lon[0]), float(lat[0]), marker="o", color='red', transform=crs.PlateCarree())
...
# Input map information
   ...
   profile_name.append(sys.argv[3])
   lats.append(sys.argv[4])
   lons.append(sys.argv[5])

# Create input dictionary for each map and pass it to the map_mslp function above

   input_dict = {}
   input_dict["latitudes"] = lats
   input_dict["longitudes"] = lons
   input_dict["infile"] = wrf_fil
   input_dict["locationname"] = profile_name[0]

   fig = map_profilelocation(input_dict)

   if fig:

      plt.savefig(dest_dir+"/profilelocation_"+dom+"_"+profile_name[0]+".png", bbox_inches='tight')
