...
# Read projection from a variable (will be able to detect all possible WRF projections and use them for plotting) also identify the required max and min indices
      if x1_y1[1] >= 0 and x1_y1[1] < np.shape(slp_all)[0] and x1_y1[0] >= 0 and x1_y1[0] < np.shape(slp_all)[1] and x2_y2[1] >= 0 and x2_y2[1] < np.shape(slp_all)[0] and x2_y2[0] >= 0 and x2_y2[0] < np.shape(slp_all)[1]:
         ...
         cloudfrac_low =  getvar(wrf_in, 'cloudfrac', timeidx=i)[0, min_lat_ind:max_lat_ind+1, min_lon_ind:max_lon_ind+1]*100.0  #Red
         cloudfrac_med =  getvar(wrf_in, 'cloudfrac', timeidx=i)[1, min_lat_ind:max_lat_ind+1, min_lon_ind:max_lon_ind+1]*100.0 #Green
         cloudfrac_high = getvar(wrf_in, 'cloudfrac', timeidx=i)[2, min_lat_ind:max_lat_ind+1, min_lon_ind:max_lon_ind+1]*100.0 #Blue
...
# Plot theta_e
         ...
         cloudfrac_lvls = [0.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0, 55.0, 60.0, 65.0, 70.0, 75.0, 80.0, 85.0, 90.0, 95.0, 100.0]
         red1 = [1,0,0,0]
         red2 = [1,0,0,0.5]
         green1 = [0,1,0,0]
         green2 = [0,1,0,0.5]
         ...
         red_cmap = ListedColormap(np.linspace(red1, red2, 21))
         low_cloud = plt.contourf(lons, lats, cloudfrac_low, levels=cloudfrac_lvls, cmap = red_cmap, antialiased=True, transform=crs.PlateCarree())

         green_cmap = ListedColormap(np.linspace(green1, green2, 21))
         med_cloud = plt.contourf(lons, lats, cloudfrac_med, levels=cloudfrac_lvls, cmap = green_cmap, antialiased=True, transform=crs.PlateCarree())

         blue_cmap = ListedColormap(np.linspace(blue1, blue2, 21))
         med_cloud = plt.contourf(lons, lats, cloudfrac_high, levels=cloudfrac_lvls, cmap = blue_cmap, antialiased=True, transform=crs.PlateCarree())
...
# Create inset colourbar
         ...
         colorbartext= "Cloud fraction (%), low cloud = red, mid cloud = green, high cloud = blue"
         ...
         if cbar_inset:
            if portrait:
               ...
               cbbox.text(0.95,0.5, colorbartext, rotation=90.0, verticalalignment='center', horizontalalignment='center')
               ...
               cbbox.tick_params(axis='both', left=False, top=False, right=False, bottom=False, labelleft=False, labeltop=False, labelright=False, labelbottom=False)
               cbbox.set_facecolor([1,1,1,0.7])
               cbbox.text(0.5,0.15, colorbartext, verticalalignment='center', horizontalalignment='center')
#               cbaxes1 = inset_axes(cbbox, '95%', '10%', loc = 9)
               cbaxes1 = inset_axes(cbbox, '95%', '95%',  bbox_to_anchor=(0,0.9,1,0.1), bbox_transform=cbbox.transAxes, loc = 9)
#              cb1 = ColorbarBase(ax=cbaxes1, cmap=red_cmap, orientation='horizontal', ticks=[])
#              cbaxes2 = inset_axes(cbbox, '95%', '10%', loc = 9)
               cb1 = ColorbarBase(ax=cbaxes1, cmap=red_cmap, orientation='horizontal', ticks=[])
 
               cbaxes2 = inset_axes(cbbox, '95%', '95%',  bbox_to_anchor=(0,0.8,1,0.1), bbox_transform=cbbox.transAxes, loc = 9)
               cb2 = ColorbarBase(ax=cbaxes2, cmap=green_cmap, orientation='horizontal', ticks=[])

               cbaxes3 = inset_axes(cbbox, '95%', '95%',  bbox_to_anchor=(0,0.7,1,0.1), bbox_transform=cbbox.transAxes, loc = 9)
               cb2 = ColorbarBase(ax=cbaxes3, cmap=blue_cmap, orientation='horizontal')
...
#            cbaxes2 = inset_axes(cbbox, '100%', '100%',  bbox_to_anchor=[0,0.6,1,0.1])
#            cb2 = ColorbarBase(ax=cbaxes2, cmap=green_cmap, orientation='horizontal', ticks=[])
...
# Define destination directory
   dest_dir = "/home/earajr/FORCE_WRF_plotting/output/cloudfrac"
...
# Loop through maps, create input dictionary for each map and pass it to the map_ccloudfraction function above
   for i in np.arange(0, np.shape(limit_lats)[0], 1):
      ...
      fig = map_cloudfraction(input_dict)

      plt.savefig(dest_dir+"/cloudfractiontest_"+map_names[i][0]+".png", bbox_inches='tight')
