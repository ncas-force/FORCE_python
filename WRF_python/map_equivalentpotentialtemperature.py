...
# Plot theta_e
         ...
         theta_e_contour = plt.contour(lons, lats, theta_e_level,levels=theta_e_lvl2, colors='k', transform=crs.PlateCarree())
         plt.clabel(theta_e_contour, inline=1, fontsize=10, fmt="%.0f")
...
# Loop through maps, create input dictionary for each map and pass it to the map_equivalentpotentialtemperature function above
   ...
   for i in np.arange(0, np.shape(limit_lats)[0], 1):
      ...
      plt.savefig(dest_dir+"/equivalentpotentialtemperaturetest_"+map_names[i][0]+".png", bbox_inches='tight')
