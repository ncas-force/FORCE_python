...
# Read projection from a variable (will be able to detect all possible WRF projections and use them for plotting) also identify the required max and min indices
      if x1_y1[1] >= 0 and x1_y1[1] < np.shape(slp_all)[0] and x1_y1[0] >= 0 and x1_y1[0] < np.shape(slp_all)[1] and x2_y2[1] >= 0 and x2_y2[1] < np.shape(slp_all)[0] and x2_y2[0] >= 0 and x2_y2[0] < np.shape(slp_all)[1]:
         ...
         omega = getvar(wrf_in, 'omega', timeidx=i)[:,min_lat_ind:max_lat_ind+1, min_lon_ind:max_lon_ind+1]
...
# Interpolate to level on correct level type
         ...
         if leveltype == "pressure":
            omega_level = interplevel(omega, pres, levels)
         if leveltype == "altitude":
            omega_level = interplevel(omega, z, levels)
...
#         omega_lvl2 = np.arange(-9.0, 11., 2.0)
...
# Create inset colourbar
         ...
         if leveltype == "pressure":
            colorbartext= "%.0f" %levels +" hPa omega (Pa $\mathregular{s^{-1}}$)"
         if leveltype == "altitude":
            colorbartext= "%.0f" %levels +" m omega (Pa $\mathregular{s^{-1}}$)"
         ...
         if cbar_inset:
            ...
            cbbox.text(0.5,0.3, colorbartext, verticalalignment='center', horizontalalignment='center')
            cbaxes = inset_axes(cbbox, '95%', '30%', loc = 9)
            cb = plt.colorbar(cax=cbaxes, orientation='horizontal', ticks=np.arange(-5.0, 6.0, 1.0))
...
# Define destination directory
   dest_dir = "/home/earajr/FORCE_WRF_plotting/output/omegalevels"
...
# Loop through maps, create input dictionary for each map and pass it to the map_omgealevels function above
   ...
   for i in np.arange(0, np.shape(limit_lats)[0], 1):
      ...
      fig = map_omega(input_dict)

      plt.savefig(dest_dir+"/omegalevelstest_"+map_names[i][0]+".png", bbox_inches='tight')
