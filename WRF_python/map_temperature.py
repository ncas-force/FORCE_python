...
# Plot temperature
         ...
         temp_contour = plt.contour(lons, lats, temp_level,levels=temp_lvl2, colors='k', transform=crs.PlateCarree())
         plt.clabel(temp_contour, inline=1, fontsize=10, fmt="%.0f")

         temp_lvl = np.arange(-80.0, 36.0, 1.0)
...
# Loop through maps, create input dictionary for each map and pass it to the map_temperature function above
   ...
   for i in np.arange(0, np.shape(limit_lats)[0], 1):
      ...
      plt.savefig(dest_dir+"/temperaturetest_"+map_names[i][0]+".png", bbox_inches='tight')
