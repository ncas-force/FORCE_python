...
# Interpolate to level on correct level type
         ...
         if leveltype == "pressure":
            rh_level = interplevel(rh, pres, levels)
         if leveltype == "altitude":
            rh_level = interplevel(rh, z, levels)
...
# Create inset colourbar
         ...
         if leveltype == "pressure":
            colorbartext= "%.0f" %levels +" hPa relative humidity (%)"
         if leveltype == "altitude":
            colorbartext= "%.0f" %levels +" m relative humidity (%)"
...
# Create input dictionary for each map and pass it to the map_relativehumidity function above
   ...
   plt.savefig(dest_dir+"/relativehumidity_"+dom+"_"+date+"_"+time+"_"+level_flag+"_"+map_names[0]+".png", bbox_inches='tight')
