def crosssection_relativehumidity(x):
...
# Compute vertical cross section interpolation
         ...
         for k in np.arange(0, np.size(gc_dist), 1):
            rh_cross.append(vertcross(rh, ht, wrfin=wrf_in, levels=interp_levs, start_point=xsection_locs[k], end_point=xsection_locs[k+1], latlon=True, meta=True))
...
# Fill in nans at the bottom of the cross section with nearest value from above
            rh_cross_np = fill_xsec_nans.fill_xsec_nans(rh_cross_np)

# Create contours 
            t_contours = ax.contourf(x_ticks, v_ticks, rh_cross_np, levels=rh_lvls, cmap='GnBu')
            t_contours2 = ax.contour(x_ticks, v_ticks, rh_cross_np, levels=rh_lvls2, colors='k')
...
#   dest_dir = "/home/earajr/FORCE_WRF_plotting/output/relhumxsection"
   dest_dir = sys.argv[2]
...
# Create input dictionary for each cross section and pass it to the crossection_trelativehumidit function above
   ...
   fig = crosssection_relativehumidity(input_dict)

   if fig:

      plt.savefig(dest_dir+"/relativehumidity_crosssection_"+dom+"_"+date+"_"+time+"_"+xsection_ids[0]+".png", bbox_inches='tight')
