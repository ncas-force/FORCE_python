def map_liftingcondensationlevel(x):
...
# Loop through maps, create input dictionary for each map and pass it to the map_lcl function above
   for i in np.arange(0, np.shape(limit_lats)[0], 1):
      ...
      fig = map_liftingcondensationlevel(input_dict)

      plt.savefig(dest_dir+"/liftingcondensationlevel_"+dom+"_"+date+"_"+time+"_"+map_names[i][0]+".png", bbox_inches='tight')
