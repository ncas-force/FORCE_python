...
# Read height, temperature and terrain
         ...
         omega = getvar(wrf_in, 'omega', timeidx=j)
...
# Compute vertical cross section interpolation
         ...
         for k in np.arange(0, np.size(gc_dist), 1):
            omega_cross.append(vertcross(omega, ht, wrfin=wrf_in, levels=interp_levs, start_point=xsection_locs[k], end_point=xsection_locs[k+1], latlon=True, meta=True))
...
# Read projection from a variable (will be able to detect all possible WRF projections and use them for plotting) 
         cart_proj = get_cartopy(omega)
         lats, lons = latlon_coords(omega)
...
# Set contour levels
         omega_lvls = np.arange(cmin, cmax, cstep)
         omega_lvls2 = np.arange(cmin, cmax, 2.0*cstep)

# loop through parts of cross section and plot data
         for k in np.arange(0, np.size(gc_dist), 1):
            ax = fig.add_subplot(gs[k])
            ax.axis(ymin=0.0, ymax=100.0)
            vert_vals = to_np(omega_cross[k].coords["vertical"])
            v_ticks = np.arange(vert_vals.shape[0])
            thin = [int(x/10.0) for x in v_ticks.shape]
            ax.set_yticks(v_ticks[::thin[0]])
            x_ticks = np.arange(0.0, np.shape(omega_cross[k])[1], 1)

            if k == 0:
               if wind_flag and np.size(gc_dist) <= 1:
                  plt_title = 'Omega cross section and wind vectors parallel to cross section:'
               else:
                  plt_title = 'Omega cross section:'
            ...
            omega_cross_np = np.where(np.isnan(to_np(omega_cross[k])), np.nan ,to_np(omega_cross[k]))
...
# Fill in nans at the bottom of the cross section with nearest value from above
            omega_cross_np = fill_xsec_nans.fill_xsec_nans(omega_cross_np)

# Create contours 
            t_contours = ax.contourf(x_ticks, v_ticks, omega_cross_np, levels=omega_lvls, cmap='bwr', extend="both")
            t_contours2 = ax.contour(x_ticks, v_ticks, omega_cross_np, levels=omega_lvls2, colors='k')
...
# Setup tickmarks so that only the first panel has y axis labels and x axis labels are lat and lon values      
            ...
            x_tick_step = round(np.shape(omega_cross_np)[1]/num_x_ticks)
            indices = np.arange(0, np.shape(omega_cross[k])[1], x_tick_step)
            while True:
               if np.shape(omega_cross_np)[1] - indices[-1] < x_tick_step*0.75:
                  step += 1
                  num_x_ticks = dist_percent[k]/step
                  x_tick_step = round(np.shape(omega_cross_np)[1]/num_x_ticks)
                  indices = np.arange(0, np.shape(omega_cross_np)[1], x_tick_step)
               else:
                  break
            ...
            if k == 0 :
               ...
               coords = to_np(omega_cross[k].coords["xy_loc"])
               ...
               labels = [str(lat_labels_str[x])+"\n"+str(lon_labels_str[x]) for x in np.arange(0, np.shape(omega_cross[k])[1], x_tick_step)]
...
# Add additional panel to act as a colorbar
         ...
         fig.colorbar(t_contours, cax=ax, label="Omega Pa/s")
...
#  dest_dir = "/home/earajr/FORCE_WRF_plotting/output/omega"
   dest_dir = sys.argv[2]
...
# Create input dictionary for each cross section and pass it to the crossection_omega function above
   ...
   fig = crosssection_omega(input_dict)

   if fig:

      plt.savefig(dest_dir+"/omega_crosssection_"+dom+"_"+date+"_"+time+"_"+xsection_ids[0]+".png", bbox_inches='tight')
