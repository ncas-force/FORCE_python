...
# Read projection from a variable (will be able to detect all possible WRF projections and use them for plotting) also identify the required max and min indices
      if x1_y1[1] >= 0 and x1_y1[1] < np.shape(slp_all)[0] and x1_y1[0] >= 0 and x1_y1[0] < np.shape(slp_all)[1] and x2_y2[1] >= 0 and x2_y2[1] < np.shape(slp_all)[0] and x2_y2[0] >= 0 and x2_y2[0] < np.shape(slp_all)[1]:
         ...
         cape3d = getvar(wrf_in, 'cape_3d', timeidx=i)[0,:,min_lat_ind:max_lat_ind+1, min_lon_ind:max_lon_ind+1]
...
# Interpolate to level on correct level type
         ...
         if leveltype == "pressure":
            cape3d_level = interplevel(cape3d, pres, levels)
         if leveltype == "altitude":
            cape3d_level = interplevel(cape3d, z, levels)
...
# Plot theta_e
         ...
         cape3d_lvls = [60, 80, 100, 120, 150, 175, 200, 240, 290, 340, 400, 500, 600, 700, 850, 1000, 1250, 1500, 1750, 2000, 2500]
         plt.contourf(lons, lats, cape3d_level, levels=cape3d_lvls, cmap='magma_r', transform=crs.PlateCarree())
...
# Create inset colourbar
         ...
         if leveltype == "pressure":
            colorbartext= "%.0f" %levels +" hPa convective available potential energy (j/Kg)"
         if leveltype == "altitude":
            colorbartext= "%.0f" %levels +" m convective available potential energy (j/Kg)"
...
#   dest_dir = "/home/earajr/FORCE_WRF_plotting/output/cape3d"
   dest_dir = sys.argv[2]
...
# Create input dictionary for each map and pass it to the map_cape3d function above
   ...
   fig = map_cape3d(input_dict)

   plt.savefig(dest_dir+"/cape3d_"+dom+"_"+date+"_"+time+"_"+level_flag+"_"+map_names[0]+".png", bbox_inches='tight')
