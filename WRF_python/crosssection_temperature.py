def crosssection_temperature(x):
...
# Compute vertical cross section interpolation
         ...
         for k in np.arange(0, np.size(gc_dist), 1):
            tc_cross.append(vertcross(tc, ht, wrfin=wrf_in, levels=interp_levs, start_point=xsection_locs[k], end_point=xsection_locs[k+1], latlon=True, meta=True))
...
# Set contour levels
         tc_lvls = np.arange(cmin, cmax, cstep)
         tc_lvls2 = np.arange(cmin+2.5, cmax, 2.0*cstep)

# loop through parts of cross section and plot data
         for k in np.arange(0, np.size(gc_dist), 1):
            ax = fig.add_subplot(gs[k])
            ax.axis(ymin=0.0, ymax=100.0)
            vert_vals = to_np(tc_cross[k].coords["vertical"])
            v_ticks = np.arange(vert_vals.shape[0])
            thin = [int(x/10.0) for x in v_ticks.shape]
            ax.set_yticks(v_ticks[::thin[0]])
            x_ticks = np.arange(0.0, np.shape(tc_cross[k])[1], 1)

            if k == 0:
               if wind_flag and np.size(gc_dist) <= 1:
                  plt_title = 'Temperature cross section and wind vectors parallel to cross section:'
               else:
                  plt_title = 'Temperature cross section:'
            ...
            tc_cross_np = np.where(np.isnan(to_np(tc_cross[k])), np.nan ,to_np(tc_cross[k]))
...
# Fill in nans at the bottom of the cross section with nearest value from above
            tc_cross_np = fill_xsec_nans.fill_xsec_nans(tc_cross_np)

# Create contours 
            t_contours = ax.contourf(x_ticks, v_ticks, tc_cross_np, levels=tc_lvls, cmap='viridis', extend="min")
            t_contours2 = ax.contour(x_ticks, v_ticks, tc_cross_np, levels=tc_lvls2, colors='k')
...
# Setup tickmarks so that only the first panel has y axis labels and x axis labels are lat and lon values
            ...
            x_tick_step = round(np.shape(tc_cross_np)[1]/num_x_ticks)
            indices = np.arange(0, np.shape(tc_cross[k])[1], x_tick_step)
            while True:
               if np.shape(tc_cross_np)[1] - indices[-1] < x_tick_step*0.75:
                  step += 1
                  num_x_ticks = dist_percent[k]/step
                  x_tick_step = round(np.shape(tc_cross_np)[1]/num_x_ticks)
                  indices = np.arange(0, np.shape(tc_cross_np)[1], x_tick_step)
               else:
                  break
            ...
            if k == 0 :
               ...
               coords = to_np(tc_cross[k].coords["xy_loc"])
               ...
               labels = [str(lat_labels_str[x])+"\n"+str(lon_labels_str[x]) for x in np.arange(0, np.shape(tc_cross[k])[1], x_tick_step)]
...
# Add additional panel to act as a colorbar
         ...
         fig.colorbar(t_contours, cax=ax, label="Temperature $^\circ$C", ticks=[-80.0, -70.0, -60.0, -50.0, -40.0, -30.0, -20.0, -10.0, 0.0, 5.0, 10.0, 15.0, 20.0])
