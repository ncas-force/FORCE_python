def crosssection_reflectivity(x):
...
# loop through parts of cross section and plot data
         for k in np.arange(0, np.size(gc_dist), 1):
            ...
            if k == 0:
               if wind_flag:
                  plt_title = 'Reflectivity cross section and wind vectors parallel to cross section:'
               else:
                  plt_title = 'Reflectivity cross section:'
...
# Fill in nans at the bottom of the cross section with nearest value from above
            dbz_cross_np = fill_xsec_nans.fill_xsec_nans(dbz_cross_np)

# Create contours
            dbz_contours = ax.contourf(x_ticks, v_ticks, dbz_cross_np, levels=dbz_lvls, cmap='jet')
            dbz_contours2 = ax.contour(x_ticks, v_ticks, dbz_cross_np, levels=dbz_lvls, colors='k')
...
# correct waypoint altitudes so that they cannot be below the suface (unless they are set to 0)
...
# Setup tickmarks so that only the first panel has y axis labels and x axis labels are lat and lon values
            ...
            x_tick_step = round(np.shape(dbz_cross_np)[1]/num_x_ticks)
            indices = np.arange(0, np.shape(dbz_cross[k])[1], x_tick_step)
            while True:
               if np.shape(dbz_cross_np)[1] - indices[-1] < x_tick_step*0.75:
                  step += 1
                  num_x_ticks = dist_percent[k]/step
                  x_tick_step = round(np.shape(dbz_cross_np)[1]/num_x_ticks)
                  indices = np.arange(0, np.shape(dbz_cross_np)[1], x_tick_step)
               else:
                  break
            ...
            if k == 0 :
               ...
               coords = to_np(dbz_cross[k].coords["xy_loc"])
               ...
               labels = [str(lat_labels_str[x])+"\n"+str(lon_labels_str[x]) for x in np.arange(0, np.shape(dbz_cross[k])[1], x_tick_step)]
...
# Set position of contour labels so that they only occur in panels that are greater than 15 % of the total width and occur down tha middle of the panel
            if any(span >= 15 for span in dist_percent_int):
               if dist_percent_int[k] >= 15:
                  ...
                  clab_locs = []
                  for line in dbz_contours2.collections:
                  ...
                  ax.clabel(dbz_contours2, fontsize=9, inline=1, fmt='%2.1f', manual=clab_locs)
            else:
...
# Add additional panel to act as a colorbar
         ...
         fig.colorbar(dbz_contours, cax=ax, label="Reflectivity dBZ")
...
# Create input dictionary for each cross section and pass it to the crossection_reflectivity function above
   ...
   fig = crosssection_reflectivity(input_dict)

   if fig:

      plt.savefig(dest_dir+"/reflectivity_crosssection_"+dom+"_"+date+"_"+time+"_"+xsection_ids[0]+".png", bbox_inches='tight')
