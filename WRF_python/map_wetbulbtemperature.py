...
# Interpolate to level on correct level type
         ...
         if leveltype == "pressure":
            twb_level = interplevel(twb, pres, levels)
         if leveltype == "altitude":
            twb_level = interplevel(twb, z, levels)
...
# Create inset colourbar
         ...
         if leveltype == "pressure":
            colorbartext= "%.0f" %levels +" hPa wet bulb temperature ($^\circ$C)"
         if leveltype == "altitude":
            colorbartext= "%.0f" %levels +" m wet bulb temperature ($^\circ$C)"
...
# Create input dictionary for each map and pass it to the map_wetbulbtemperature function above
   ...
   fig = map_wetbulbtemperature(input_dict)

   plt.savefig(dest_dir+"/wetbulbtemperature_"+dom+"_"+date+"_"+time+"_"+level_flag+"_"+map_names[0]+".png", bbox_inches='tight')
