...
# Read pressure, theta_e and winds
         geopotential = getvar(wrf_in, 'geopotential', timeidx=i)[:,min_lat_ind:max_lat_ind+1, min_lon_ind:max_lon_ind+1]

         w = getvar(wrf_in, 'wa', timeidx=i, units='m/s')[:,min_lat_ind:max_lat_ind+1, min_lon_ind:max_lon_ind+1]
...
# Interpolate to level on correct level type
         ...
         if leveltype == "pressure":
            geopotential_level = interplevel(geopotential, pres, levels)
            w_level = interplevel(w, pres, levels)
            u_level = interplevel(u, pres, levels)
            v_level = interplevel(v, pres, levels)
         if leveltype == "altitude":
            geopotential_level = interplevel(geopotential, z, levels)
            w_level = interplevel(w, z, levels)
            u_level = interplevel(u, z, levels)
            v_level = interplevel(v, z, levels)
         
#         windspeed_level = np.sqrt((u_level**2.0) + (v_level**2.0))
...
#         plt.clabel(rh_contour, inline=1, fontsize=10, fmt="%g")
         ...
         w_lvl = np.arange(-0.25, 0.2725, 0.025)
         plt.contourf(lons, lats, w_level, levels=w_lvl, zorder=1, cmap='RdBu_r', transform=crs.PlateCarree(), extend="both")
...
# Create inset colourbar
         ...
         if leveltype == "pressure":
            colorbartext= "%.0f" %levels +" hPa vertical windspeed (m/s)"
         if leveltype == "altitude":
            colorbartext= "%.0f" %levels +" m vertical windspeed (m/s)"
...
#   dest_dir = "/home/earajr/FORCE_WRF_plotting/output/relativehumidity"
   dest_dir = sys.argv[2]
...
# Create input dictionary for each map and pass it to the map_w function above
   ...
   fig = map_w(input_dict)

   plt.savefig(dest_dir+"/w_"+dom+"_"+date+"_"+time+"_"+level_flag+"_"+map_names[0]+".png", bbox_inches='tight')
