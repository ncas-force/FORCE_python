...
# Create inset colourbar
         ...
         if cbar_inset:
            ...
            if portrait:
               ...
               cbbox.tick_params(axis='both', left=False, top=False, right=False, bottom=False, labelleft=False, labeltop=False, labelright=False, labelbottom=False)
               cbbox.set_facecolor([1,1,1,0.7])
               cbbox.text(0.5,0.3, "Lifting condensation level (m)", verticalalignment='center', horizontalalignment='center')
...
# Loop through maps, create input dictionary for each map and pass it to the map_lfc function above
   for i in np.arange(0, np.shape(limit_lats)[0], 1):
      ...
      fig = map_lfc(input_dict)

      plt.savefig(dest_dir+"/lfctest_"+map_names[i][0]+".png", bbox_inches='tight')
