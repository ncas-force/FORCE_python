...
# Plot 2m temperature
         T2_lvl = np.arange(-15.5, 40.5, 0.5)
         plt.contourf(lons, lats, T2, levels=T2_lvl, cmap='gist_ncar', transform=crs.PlateCarree())
...
# Loop through maps, create input dictionary for each map and pass it to the map_2mtemperature function above
   ...
   for i in np.arange(0, np.shape(limit_lats)[0], 1):
      ...
      plt.savefig(dest_dir+"/2mtemperature_"+dom+"_"+date+"_"+time+"_"+map_names[i][0]+".png", bbox_inches='tight')
