def map_potentialtemperature(x):
   import numpy as np
...
# Read projection from a variable (will be able to detect all possible WRF projections and use them for plotting) also identify the required max and min indices
      if x1_y1[1] >= 0 and x1_y1[1] < np.shape(slp_all)[0] and x1_y1[0] >= 0 and x1_y1[0] < np.shape(slp_all)[1] and x2_y2[1] >= 0 and x2_y2[1] < np.shape(slp_all)[0] and x2_y2[0] >= 0 and x2_y2[0] < np.shape(slp_all)[1]:
         ...
         theta = getvar(wrf_in, 'theta', timeidx=i)[:,min_lat_ind:max_lat_ind+1, min_lon_ind:max_lon_ind+1]
...
# Interpolate to level on correct level type
         ...
         if leveltype == "pressure":
            theta_level = interplevel(theta, pres, levels)
         if leveltype == "altitude":
            theta_level = interplevel(theta, z, levels)
...
# Create inset colourbar
         ...
         if leveltype == "pressure":
            colorbartext= "%.0f" %levels +" hPa potential temperature (K)"
         if leveltype == "altitude":
            colorbartext= "%.0f" %levels +" m potential temperature (K)"
...
# Define destination directory
   dest_dir = "/home/earajr/FORCE_WRF_plotting/output/potentialtemperature"
...
# Loop through maps, create input dictionary for each map and pass it to the map_potentialtemperature function above
   ...
   for i in np.arange(0, np.shape(limit_lats)[0], 1):
      ...
      plt.savefig(dest_dir+"/potentialtemperaturetest_"+map_names[i][0]+".png", bbox_inches='tight')
