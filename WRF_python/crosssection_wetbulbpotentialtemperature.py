def crosssection_wetbulbpotentialtemperature(x):
         ...
         for k in np.arange(0, np.size(gc_dist), 1):
            ...
            if k == 0:
               if wind_flag and np.size(gc_dist) <= 1:
                  plt_title = 'Wetbulb Potential Temperature cross section and wind vectors parallel to cross section:'
               else:
                  plt_title = 'Wetbulb Potential Temperature cross section:'
...
# Setup tickmarks so that only the first panel has y axis labels and x axis labels are lat and lon values      
            ...
            x_tick_step = round(np.shape(theta_w_cross_np)[1]/num_x_ticks)
            indices = np.arange(0, np.shape(theta_w_cross[k])[1], x_tick_step)
            while True:
               if np.shape(theta_w_cross_np)[1] - indices[-1] < x_tick_step*0.75:
                  step += 1
                  num_x_ticks = dist_percent[k]/step
                  x_tick_step = round(np.shape(theta_w_cross_np)[1]/num_x_ticks)
                  indices = np.arange(0, np.shape(theta_w_cross_np)[1], x_tick_step)
               else:
                  break
            ...
            if k == 0 :
               ...
               coords = to_np(theta_w_cross[k].coords["xy_loc"])
               ...
               labels = [str(lat_labels_str[x])+"\n"+str(lon_labels_str[x]) for x in np.arange(0, np.shape(theta_w_cross[k])[1], x_tick_step)]
...
# Create input dictionary for each cross section and pass it to the crossection_wetbulbpotential function above
   ...
   fig = crosssection_wetbulbpotentialtemperature(input_dict)

   if fig:

      plt.savefig(dest_dir+"/wetbulbpotentialtemperature_crosssection_"+dom+"_"+date+"_"+time+"_"+xsection_ids[0]+".png", bbox_inches='tight')
