def map_2mdewpointdepression(x):
...
# Read projection from a variable (will be able to detect all possible WRF projections and use them for plotting) also identify the required max and min indices 
      if x1_y1[1] >= 0 and x1_y1[1] < np.shape(slp_all)[0] and x1_y1[0] >= 0 and x1_y1[0] < np.shape(slp_all)[1] and x2_y2[1] >= 0 and x2_y2[1] < np.shape(slp_all)[0] and x2_y2[0] >= 0 and x2_y2[0] < np.shape(slp_all)[1]:
         ...
         ht = getvar(wrf_in, 'z', timeidx=i)[:,min_lat_ind:max_lat_ind+1, min_lon_ind:max_lon_ind+1]
...
# Create inset colourbar
         ...
         if cbar_inset:
            ...
            if portrait:
               ...
               cbbox.text(0.85,0.5, "2m dewpoint depression (K)", rotation=90.0, verticalalignment='center', horizontalalignment='center')
               ...
               cbbox.tick_params(axis='both', left=False, top=False, right=False, bottom=False, labelleft=False, labeltop=False, labelright=False, labelbottom=False)
               cbbox.set_facecolor([1,1,1,0.7])
               cbbox.text(0.5,0.1, "2m dewpoint depression (K)", verticalalignment='center', horizontalalignment='center')
            ...
            if portrait:
               ...
               cbbox.tick_params(axis='both', left=False, top=False, right=False, bottom=False, labelleft=False, labeltop=False, labelright=False, labelbottom=False)
               cbbox.set_facecolor([1,1,1,0.7])
               cbbox.text(0.5,-0.2, "2m dewpoint depression (K)", verticalalignment='center', horizontalalignment='center')
               cbaxes = inset_axes(cbbox, '95%', '30%', loc = 9)
               cb = plt.colorbar(cax=cbaxes, orientation='horizontal')
...
# Create input dictionary for each map and pass it to the map_2mdewpointdepression function above
   ...
   fig = map_2mdewpointdepression(input_dict)

   plt.savefig(dest_dir+"/2mdewpointdepression_"+dom+"_"+date+"_"+time+"_"+map_names[0]+".png", bbox_inches='tight')
