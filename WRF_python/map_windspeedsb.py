...
#         plt.clabel(rh_contour, inline=1, fontsize=10, fmt="%g")
         ...
         windspeed_lvl = np.arange(0.0, 10.0, 0.25)
...
# Create inset colourbar
         ...
         if leveltype == "pressure":
            colorbartext= "%.0f" %levels +" hPa windspeed (m/s)"
         if leveltype == "altitude":
            colorbartext= "%.0f" %levels +" m windspeed (m/s)"
...
# Create input dictionary for each map and pass it to the map_windspeed function above
   ...
   fig = map_windspeedsb(input_dict)

   plt.savefig(dest_dir+"/windspeedsb_"+dom+"_"+date+"_"+time+"_"+level_flag+"_"+map_names[0]+".png", bbox_inches='tight')
