...
# Read height, temperature and terrain
         ...
         spec_hum = getvar(wrf_in, 'QVAPOR', timeidx=j)*1000.0
...
# Compute vertical cross section interpolation
         ...
         for k in np.arange(0, np.size(gc_dist), 1):
            spec_hum_cross.append(vertcross(spec_hum, ht, wrfin=wrf_in, levels=interp_levs, start_point=xsection_locs[k], end_point=xsection_locs[k+1], latlon=True, meta=True))
...
# loop through parts of cross section and plot data
         for k in np.arange(0, np.size(gc_dist), 1):
            ...
            vert_vals = to_np(spec_hum_cross[k].coords["vertical"])
            v_ticks = np.arange(vert_vals.shape[0])
            thin = [int(x/10.0) for x in v_ticks.shape]
            ax.set_yticks(v_ticks[::thin[0]])
            x_ticks = np.arange(0.0, np.shape(spec_hum_cross[k])[1], 1)

            if k == 0:
               if wind_flag and np.size(gc_dist) <= 1:
                  plt_title = 'Specific Humidity cross section and wind vectors parallel to cross section:'
               else:
                  plt_title = 'Specific Humidity cross section:'
            ...
            spec_hum_cross_np = np.where(np.isnan(to_np(spec_hum_cross[k])), np.nan ,to_np(spec_hum_cross[k]))
...
# Setup tickmarks so that only the first panel has y axis labels and x axis labels are lat and lon values      
            ...
            x_tick_step = round(np.shape(spec_hum_cross_np)[1]/num_x_ticks)
            indices = np.arange(0, np.shape(spec_hum_cross[k])[1], x_tick_step)
            while True:
               if np.shape(spec_hum_cross_np)[1] - indices[-1] < x_tick_step*0.75:
                  step += 1
                  num_x_ticks = dist_percent[k]/step
                  x_tick_step = round(np.shape(spec_hum_cross_np)[1]/num_x_ticks)
                  indices = np.arange(0, np.shape(spec_hum_cross_np)[1], x_tick_step)
               else:
                  break
            ...
            if k == 0 :
               ...
               coords = to_np(spec_hum_cross[k].coords["xy_loc"])
               ...
               labels = [str(lat_labels_str[x])+"\n"+str(lon_labels_str[x]) for x in np.arange(0, np.shape(spec_hum_cross[k])[1], x_tick_step)]
...
# Add additional panel to act as a colorbar
         ...
         fig.colorbar(t_contours, cax=ax, label="Specific Humidity g/kg")
...
#   dest_dir = "/home/earajr/FORCE_WRF_plotting/output/spechumxsection"
   dest_dir = sys.argv[2]
...
# Create input dictionary for each cross section and pass it to the crossection_specifichumidity function above
   ...
   fig = crosssection_specifichumidity(input_dict)

   if fig:

      plt.savefig(dest_dir+"/specifichumidity_crosssection_"+dom+"_"+date+"_"+time+"_"+xsection_ids[0]+".png", bbox_inches='tight')
