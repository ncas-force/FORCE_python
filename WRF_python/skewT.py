...
# loop through soundings
   ...
   for j in np.arange(0, np.shape(sounding_names)[0], 1):
      ...
      if x_y[1] >= 0 and x_y[1] < np.shape(lats)[0] and x_y[0] >= 0 and x_y[0] < np.shape(lats)[1]:
         ...
         axin.set_title('Wind Profile \n 10m/s increment', fontsize=8, fontweight='bold', horizontalalignment='center')

         h = Hodograph(axin, component_range=30.)
         h.add_grid(increment=10)
         h.plot_colormapped(u_s[0:38], v_s[0:38], hgt_s[0:38])

# FORMAT FOR printing to title         print("{:.0f}".format(float(str(to_np(cape)).split()[0])))

         ax.set_title('SkewTLogP '+sounding_names[j][0]+"\nLat: "+sounding_lats[j][0]+"   Lon: "+sounding_lons[j][0]+"\nCAPE = "+ "{:.0f}".format(float(str(to_np(cape)).split()[0]))+" CIN = "+ "{:.0f}".format(float(str(to_np(cin)).split()[0])), horizontalalignment='left', fontsize=10, fontweight='bold', loc='left')
