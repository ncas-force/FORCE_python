...
# Read projection from a variable (will be able to detect all possible WRF projections and use them for plotting) also identify the required max and min indices
      if x1_y1[1] >= 0 and x1_y1[1] < np.shape(slp_all)[0] and x1_y1[0] >= 0 and x1_y1[0] < np.shape(slp_all)[1] and x2_y2[1] >= 0 and x2_y2[1] < np.shape(slp_all)[0] and x2_y2[0] >= 0 and x2_y2[0] < np.shape(slp_all)[1]:
         ...
         cin3d = getvar(wrf_in, 'cape_3d', timeidx=i)[1,:,min_lat_ind:max_lat_ind+1, min_lon_ind:max_lon_ind+1]*-1.0
...
# Interpolate to level on correct level type
         ...
         if leveltype == "pressure":
            cin3d_level = interplevel(cin3d, pres, levels)
         if leveltype == "altitude":
            cin3d_level = interplevel(cin3d, z, levels)
...
# Plot theta_e
         ...
         cin3d_lvls = [-275, -250, -225, -200, -175, -150, -125, -100, -75, -50, -25, 0]
         plt.contourf(lons, lats, cin3d_level, levels=cin3d_lvls,  cmap="magma", transform=crs.PlateCarree())
...
# Create inset colourbar
         ...
         if leveltype == "pressure":
            colorbartext= "%.0f" %levels +" hPa convective inhibition (j/Kg)"
         if leveltype == "altitude":
            colorbartext= "%.0f" %levels +" m convective inhibition (j/Kg)"
...
#   dest_dir = "/home/earajr/FORCE_WRF_plotting/output/cin3d"
   dest_dir = sys.argv[2]
...
# Create input dictionary for each map and pass it to the map_cin3d function above
   ...
   fig = map_cin3d(input_dict)

   plt.savefig(dest_dir+"/cin3d_"+dom+"_"+date+"_"+time+"_"+level_flag+"_"+map_names[0]+".png", bbox_inches='tight')
